...
from dataset.lazy_loader import LazyLoader, Celeba, MAFL
...
def gan_trainer(model, generator, decoder, encoder_HG, style_encoder, R_s, style_opt, heatmaper, g_transforms):
    def gan_train(i, real_img, pred_measures, sparse_hm, apply_g=True):
        ...
        if True:
            ...
            (
                    writable("BCE content gan", stariy_hm_loss)(fake_content, trans_sparse_hm, 5000) * coefs[
                "BCE content gan"] +
...
def content_trainer_supervised(cont_opt, encoder_HG, loader):
    ...
    def do_train():
        ...
        w300_target_hm = heatmaper.forward(w300_mes.probability.type(torch.float32),
                                           w300_mes.coord.type(torch.float32) * 63).detach()
        content300 = encoder_HG(w300_image)

        coefs = json.load(open("../parameters/content_loss.json"))

        writable("W300 Loss", hm_svoego_roda_loss)(content300, w300_target_hm).__mul__(
            coefs["borj4_w300"]).minimize_step(cont_opt)
